...
from Tiles_not2 import tiles, backgrounds_styles, backgrounds
...
class Player(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        global hint_bool
        global key_bool
        global key_count_invis
        #global next_level_bool
        ...
        if game_over == 0:
            ...
            if pygame.sprite.spritecollide(self, key_group, True):
                ...
                print('key')
            if key_bool:
                if pygame.sprite.spritecollide(self, end_game_group, False):
                    #game_over = -1
                    print('exit_True')
            else:
                if pygame.sprite.spritecollide(self, end_game_group, False):
            ...
            #        num += 1
            #        next_level_bool = False
            #        load_level_from_file(world, f'level_{num}.txt')
            #        print(num)
    ...
    def __init__(self, data):
        ...
        for row_count, row in enumerate(data):
            for count, tile in enumerate(row):
                if tile != 0 and tile != 4 and tile != 6 and tile != 7 and tile != 8 and tile != 9 and tile in tiles:
                    img = None
                    if tile in tiles:
                ...
                if tile == 7:
                    ...
                    coin_group.add(coin)
                if tile == 8:
                    end = Next_level(count * tile_size + (tile_size // 2), row_count * tile_size + (tile_size // 2))
                    end_game_group.add(end)
                if tile == 9:
...
class Next_level(pygame.sprite.Sprite):
    def __init__(self, x, y):
        pygame.sprite.Sprite.__init__(self)
        img = pygame.image.load('img/Tiles/tile_0112.png')
...
load_level_from_file(world_list, f'level_0.txt')
...
if __name__ == '__main__':
    ...
    font_key_score = pygame.font.SysFont('key_counter', 30)
    font_start = pygame.font.SysFont('start', 70)
    font_dev = pygame.font.SysFont('dev', 30)
    font_hint = pygame.font.SysFont('hint', 20)
    ...
    while run:
        ...
        if main_menu is True:
            ...
            if not key_bool:
                draw_text(f'{str(key_count)}/3', font_key_score, red, screen_width - 60, 520)
            else:
                draw_text(f'{str(key_count)}/3', font_key_score, green, screen_width - 60, 520)
            if key_count < 3 and hint_bool:
                draw_text(f'Сначала собери ключи', font_hint, red, screen_width - 200, 8)
            if key_count_invis == 3:
            ...
            if game_over == -1:
                if restart_button.draw() and restart_button.clicked:
                    ...
                    print(score)
            if game_over == 1:
                load_level_from_file(world_list, f'level_{num + 1}.txt')
                game_over = 0
